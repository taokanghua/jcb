...
export default {
    ...
    mounted() {
        setTimeout(() => {
            // 克隆dom
            this.starDom();
            this.setTransform(this._width * -1);
    ...
    methods: {
        ...
        setTransform(num) {
            ["transform", "-webkit-transform", "-ms-transform"].forEach((v) => {
                this.dom[v] = `translate3d(${num}px, 0px, 0px)`;
            });
        ...
        starDom() {
            let target = document.querySelector("." + this.className);
            let SlideDom = target.getElementsByClassName("wh_slide");
            ...
            target.insertBefore(cloneDom2, SlideDom[0]);
            target.appendChild(cloneDom1);
            this._width = target.offsetWidth;
            this.dom = target.style;
        },
        clearTimeOut() {
            this.auto = false;
            window.clearTimeout(this.timer1);
